...
Any user with the `plugins` permission could clone any post of the site, other authors' drafts included; cloning now also needs the right to edit the source post and to create posts of its type, and the editor shows the clone link only to such a user. The clone is the cloner's own post, as a created post is. A published post clones as pending for a user who cannot publish posts of its type, as core holds every other path to `published` to that right. A clone is dated by its own publish, not the source's. Core's content scan runs on the clone for a cloner without the unfiltered-content right, as on a post they create; a refused clone answers with the refusal on the source's editor instead of a 500. The copied summary is held to the scan the post editor applies to a submitted one, for such a cloner. A copied custom field value the scan refuses is named in the refusal, with its reason, instead of a bare "is invalid". A trashed source clones as pending and an autosave buffer as a draft of its own, cut from its parent, instead of copying a status the editor never offers. [#4](https://github.com/owen2345/camaleon-post-clone/pull/4).
